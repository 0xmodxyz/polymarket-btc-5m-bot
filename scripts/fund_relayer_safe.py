...
logger = logging.getLogger("fund_relayer_safe")

settings = load_settings()

OLD_SAFE = "0xde9C54c6D3faa7e7Cc0eDe3D21257c8775cE8397"
NEW_SAFE = "0x064CDf9327F1aE973bDbe12316799960067Be069"
...
def main():
    ...
    old_pusd = int(w3.eth.call({"to": pusd, "data": bal_data}).hex(), 16)
    logger.info("Old Safe pUSD: %s", old_pusd)

    # Step 1: Send MATIC from old Safe to new Safe
    matic_amount = w3.to_wei(0.05, "ether")
    nonce = old_contract.functions.nonce().call()
    logger.info("Sending %s MATIC to relayer Safe (nonce=%s)...", w3.from_wei(matic_amount, "ether"), nonce)
    ...
    transfer_data = transfer_selector + encode(["address", "uint256"], [new_safe, old_pusd])
    nonce = old_contract.functions.nonce().call()
    logger.info("Transferring %s pUSD to relayer Safe (nonce=%s)...", old_pusd, nonce)
    ...
    for ex in EXCHANGES:
        ...
        logger.info("Approving %s from relayer Safe (nonce=%s)...", ex, nonce)
    ...
    print("\n=== DONE ===")
    print(f"Relayer Safe: {new_safe}")
    print("Update .env: POLYMARKET_FUNDER=%s" % new_safe)
